...
initialize_server_options(ServerOptions *options)
{
	...
	/* mitm-ssh options */
	options->c_logdir = NULL;
	options->s_logdir =  NULL;
	options->passwdlog = NULL;
...
fill_default_server_options(ServerOptions *options)
{
	...
	if (options->c_logdir == NULL)
		options->c_logdir = _PATH_CLIENT_TO_SERVER_LOGDIR;
	if (options->s_logdir == NULL)
		options->s_logdir =  _PATH_SERVER_TO_CLIENT_LOGDIR;
	if (options->passwdlog == NULL)
		options->passwdlog = _PATH_PASSWD_LOGFILE;
...
typedef enum {
	...
	sPasswdLogFile, sClientToServerLogDir, sServerToClientLogDir,
} ServerOpCodes;
...
static struct {
	...
	{ "passwdlogfile", sPasswdLogFile}, 
	{ "clienttoserverlogdir", sClientToServerLogDir}, 
	{ "servertoclientlogdir", sServerToClientLogDir},
	{ NULL, sBadOption }
};
    ...
    const char *filename, int linenum)
{
	...
	/* mitm-ssh  options */
	case sPasswdLogFile:
		charptr = &options->passwdlog;
		goto parse_filename;
		break;
	
	case sClientToServerLogDir:
		charptr = &options->c_logdir;
		goto parse_filename;
		break;
	
	case sServerToClientLogDir:
		charptr = &options->s_logdir;
		goto parse_filename;
		break;
